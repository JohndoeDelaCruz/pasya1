...
class MapDataController extends Controller
{
    ...
    public function getMunicipalityDetails(Request $request, $municipality)
    {
        ...
        $monthlyQuery = Crop::where('municipality', $municipality);
        if ($crop) $monthlyQuery->where('crop', $crop);
        if ($year) $monthlyQuery->where('year', $year);
        ...
        $cropDistQuery = Crop::where('municipality', $municipality);
        if ($year) $cropDistQuery->where('year', $year);
        ...
        // Farm type breakdown
        $farmTypeQuery = Crop::where('municipality', $municipality);
        if ($crop) $farmTypeQuery->where('crop', $crop);
        if ($year) $farmTypeQuery->where('year', $year);

        $farmTypeBreakdown = $farmTypeQuery
            ->select('farm_type',
        ...
        // Summary statistics
        $summaryQuery = Crop::where('municipality', $municipality);
        if ($crop) $summaryQuery->where('crop', $crop);
        if ($year) $summaryQuery->where('year', $year);
    ...
    public function getTimelineData(Request $request)
    {
        ...
        foreach ($months as $month) {
            $query = Crop::query()->where('month', $month);
    ...
    public function compareData(Request $request)
    {
        ...
        foreach ($municipalities as $municipality) {
            $query = Crop::where('municipality', trim($municipality));
            if ($crop) $query->where('crop', $crop);
            if ($year) $query->where('year', $year);
    ...
    public function getStatistics()
    {
        ...
        $municipalities = Crop::distinct('municipality')->count();
        $crops = Crop::distinct('crop')->count();
        $minYear = Crop::min('year');
        $maxYear = Crop::max('year');
        ...
        // Top crops
        $topCrops = Crop::select('crop', DB::raw('SUM(production) as total'))
            ->groupBy('crop')
            ->orderBy('total', 'desc')
            ->limit(5)
            ->get();
        ...
        return response()->json([
            ...
            'statistics' => [
                ...
                'years_covered' => [
                    'min' => $minYear,
                    'max' => $maxYear,
                    'range' => ($maxYear && $minYear) ? (($maxYear - $minYear + 1) . ' years') : '0 years'
                ],
                'total_production' => round($totalProduction, 2),
                'total_area_harvested' => round($totalArea, 2),
                'avg_productivity' => round($avgProductivity ?? 0, 2),
