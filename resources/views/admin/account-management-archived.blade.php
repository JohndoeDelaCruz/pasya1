<x-admin-layout>
    ...
    <div class="p-3 sm:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600">Archived Farmers</p>
                        <p class="text-2xl font-bold text-gray-800">{{ number_format($stats['archived_farmers']) }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600">Archived Municipalities</p>
                        <p class="text-2xl font-bold text-gray-800">{{ number_format($stats['archived_municipalities']) }}</p>
                    </div>
            ...
            <form method="GET" action="{{ route('admin.farmers.archived') }}" class="flex flex-wrap gap-3 items-end">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="text"
                           ...
                           placeholder="Search archived accounts by ID, name, municipality..."
                ...
                <div>
                    <button type="submit"
                            class="bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2 px-6 rounded-lg transition flex items-center">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Archived</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($archivedFarmers as $farmer)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ optional($farmer->deleted_at)->format('M d, Y g:i A') }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-center">
                                    <form action="{{ route('admin.farmers.restore', $farmer->id) }}"
                                          method="POST"
                                          onsubmit="return confirm('Restore this farmer account? The farmer will be able to sign in again.');"
                                          class="inline">
                                        @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center text-gray-500">
                                    ...
                                    <p class="mb-3">No archived farmer accounts found</p>
                                    <a href="{{ route('admin.farmers.index') }}" class="text-green-600 hover:underline font-medium">Return to active accounts →</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($archivedFarmers->hasPages())
                <div class="px-6 py-4 border-t border-gray-200">
                    {{ $archivedFarmers->links() }}
                </div>
            @endif
        </div>
    </div>
