    ...
    <div class="h-full overflow-auto bg-gray-100" x-data="calendarApp()">
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-1 mb-2">
                            <div class="text-center text-sm font-semibold text-gray-400 py-2"
                                x-text="getDayName(selectedDate)"></div>
                        </div>

                        <!-- Day Content -->
                        <div @click="selectDayFromDate(selectedDate)" :class="{
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div x-show="showEventModal" x-transition:enter="transition ease-out duration-300"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <template x-if="selectedEvent && !selectedDay">
                                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg"
                                        ...
                                        <svg x-show="selectedEvent.type === 'harvest'" class="w-6 h-6" fill="none"
                                            stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="px-6 py-5 max-h-80 overflow-y-auto">
                        ...
                        <template x-if="selectedEvent && !selectedDay">
                            <div class="space-y-3">
                                <div class="flex items-start space-x-3 p-3 rounded-xl border"
                                    :class="getEventBgClass(selectedEvent.type)">
                                    <div class="w-3 h-3 rounded-full mt-1"
                                        :class="getEventDotClass(selectedEvent.type)"></div>
                                    <div class="flex-1">
                                    ...
                                    <template x-if="selectedEvent.crop_plan_id">
                                        <button @click="deleteCropPlan(selectedEvent.crop_plan_id)"
                                            ...
                                            title="Delete this crop plan">
                                            <svg x-show="!isDeleting" class="w-5 h-5" fill="none" stroke="currentColor"
                                                stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                            <svg x-show="isDeleting" class="animate-spin w-5 h-5" fill="none"
                                                viewBox="0 0 24 24">
                            ...
                            <div class="space-y-3">
                                <template x-for="(event, index) in selectedDay.events" :key="index">
                                    <div class="flex items-start space-x-3 p-3 rounded-xl border"
                                        ...
                                        <div class="w-3 h-3 rounded-full mt-1" :class="getEventDotClass(event.type)">
                                        </div>
                                        <div class="flex-1">
                                        ...
                                        <template x-if="event.crop_plan_id">
                                            <button @click="deleteCropPlan(event.crop_plan_id)" :disabled="isDeleting"
                                                class="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition flex-shrink-0"
                                                title="Delete this crop plan">
                                                <svg x-show="!isDeleting" class="w-5 h-5" fill="none"
                                                    stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                        <button @click="showEventModal = false; selectedEvent = null;"
                            class="w-full bg-green-500 hover:bg-green-600 text-white font-medium py-2.5 rounded-xl transition">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <div x-show="showSettingsModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div x-show="showSettingsModal" x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                    class="relative bg-white rounded-2xl shadow-xl max-w-sm w-full overflow-hidden" @click.stop>
        ...
        <script>
            function calendarApp() {
                ...
                return {
                    ...
                    async deleteCropPlan(cropPlanId) {
                        if (!confirm('Are you sure you want to delete this crop plan? This will remove all associated events (planting, harvest, and fertilizer).')) {
                            return;
                        }
                        ...
                        try {
                            ...
                            alert('Failed to delete crop plan. Error: ' + error.message);
                        } finally {
