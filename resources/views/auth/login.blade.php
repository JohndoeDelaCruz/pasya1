...
<body class="pasya-auth-shell pasya-login-page min-h-[100dvh] bg-[#f5f5f7] text-gray-900 antialiased lg:h-[100dvh] lg:overflow-hidden">
    ...
    <main class="flex min-h-[100dvh] items-center justify-center px-3 py-3 sm:px-6 sm:py-4 lg:h-[100dvh] lg:min-h-0 lg:overflow-hidden">
        <div class="pasya-login-card grid w-full max-w-5xl overflow-hidden rounded-2xl border border-black/10 bg-white lg:max-h-[calc(100dvh-2rem)] lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,.9fr)]">
            <section class="px-5 py-5 sm:px-8 sm:py-6 lg:overflow-y-auto lg:px-10 lg:py-6">
                <a href="{{ url('/') }}" class="inline-flex items-center gap-3 rounded-xl focus:outline-none focus:ring-4 focus:ring-green-100" aria-label="PASYA home">
                    <img src="{{ asset('images/PASYA.png') }}" alt="" class="h-9 w-9 object-contain sm:h-10 sm:w-10">
                    <span>
                ...
                <div class="mt-6 max-w-md sm:mt-7 lg:mt-5" x-data="{ loginMode: @js($initialLoginMode), showPassword: false, get staffMode() { return this.loginMode === 'staff' } }">
                    <p class="text-xs font-semibold text-green-700">Secure access</p>
                    <h1 class="mt-1 text-2xl font-semibold tracking-[-0.03em] text-gray-950 sm:text-3xl">Welcome back</h1>
                    <p class="mt-2 text-sm leading-5 text-gray-500">Choose your account type, then enter your PASYA account details.</p>

                    <x-auth-session-status class="mt-4 rounded-xl border border-green-200 bg-green-50 px-4 py-2.5 text-sm text-green-800" :status="session('status')" />

                    <div class="mt-5 grid grid-cols-2 gap-1 rounded-xl bg-gray-100 p-1" role="tablist" aria-label="Account type">
                        <button type="button"
                    ...
                    <form method="POST" action="{{ route('login', absolute: false) }}" class="mt-4 space-y-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <input id="email" type="text" name="email" value="{{ old('email') }}"
                                   ...
                                   class="mt-1.5 min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-base text-gray-900 placeholder:text-gray-400 focus:border-green-700 focus:ring-4 focus:ring-green-100">
                            <p class="mt-1 text-xs leading-4 text-gray-500" x-text="staffMode ? 'Use the credentials issued to your DA or LGU staff account.' : 'Use the farmer ID linked to your registered PASYA account.'"></p>
                        ...
                        <div x-show="staffMode" style="{{ $initialLoginMode === 'staff' ? '' : 'display: none;' }}">
                            ...
                            <div class="relative mt-1.5">
                                <input id="password" :type="showPassword ? 'text' : 'password'" name="password" x-ref="password"
                                       ...
                                       class="min-h-11 w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 pr-20 text-base text-gray-900 placeholder:text-gray-400 focus:border-green-700 focus:ring-4 focus:ring-green-100">
                        ...
                        <button type="submit" class="flex min-h-11 w-full items-center justify-center rounded-xl bg-[#18794e] px-5 py-2.5 text-base font-semibold text-white shadow-sm hover:bg-[#11633e] focus:outline-none focus:ring-4 focus:ring-green-100">
                            Continue
                        </button>
                    </form>
            ...
            <aside class="pasya-login-aside relative hidden overflow-hidden bg-[#153d2b] px-8 py-8 text-white lg:flex lg:flex-col lg:justify-between">
                ...
                <div class="relative">
                    ...
                    <h2 class="mt-4 max-w-sm text-3xl font-semibold leading-tight tracking-[-0.04em]">Plan clearly.<br>Respond earlier.<br>Farm with confidence.</h2>
                    <p class="mt-4 max-w-sm text-sm leading-5 text-green-100/80">Crop planning, local weather, market prices, and LGU coordination in one focused workspace.</p>
                </div>
                ...
                <div class="relative flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur-sm">
                    <a href="{{ $appDownloadUrl }}" target="_blank" rel="noopener" class="shrink-0 rounded-xl bg-white p-2" aria-label="Open PASYA app download">
                        <img src="{{ $qrCodeUrl }}" alt="QR code for the PASYA app download" class="h-16 w-16 object-contain" loading="lazy">
                    </a>
                    <div>
