    ...
    public function exportPlantingReportCsv(Request $request)
    {
        ...
        return response()->streamDownload(function () use ($plantingRecordsQuery) {
            ...
            fputcsv($handle, [
                ...
                'Actual Harvest (MT)',
                'Harvest Variance (MT)',
                'Actual Harvest Validation',
                'Actual Harvest Validator',
                'Actual Harvest Validated At',
                'Actual Harvest LGU Notes',
                'Production Loss (MT)',
                'Damaged Area (ha)',
                'Damage Cause',
                'Damage Date',
            ...
            $plantingRecordsQuery->chunk(200, function ($records) use ($handle) {
                foreach ($records as $record) {
                    ...
                    fputcsv($handle, [
                        ...
                        optional($record->actual_harvest_date)->format('Y-m-d'),
                        $record->actual_harvest_production_mt !== null ? number_format((float) $record->actual_harvest_production_mt, 4, '.', '') : '',
                        $record->actual_harvest_variance_mt !== null ? number_format((float) $record->actual_harvest_variance_mt, 4, '.', '') : '',
                        $record->actualHarvestReport?->lgu_validation_status_label ?? ($record->actual_harvest_production_mt !== null ? 'LGU Approved' : 'Not reported'),
                        $record->actualHarvestReport?->lguValidator?->name ?? '',
                        optional($record->actualHarvestReport?->lgu_validated_at)->format('Y-m-d H:i:s'),
                        $record->actualHarvestReport?->lgu_validation_notes ?? '',
                        ...
                        optional($record->damage_occurred_on)->format('Y-m-d'),
                        $record->damage_notes ?? '',
    ...
    private function getPlantingReportQuery(array $filters): Builder
    {
        return CropPlan::query()
            ->with([
                ...
                'actualHarvestReport:id,lgu_validation_status,lgu_validated_by,lgu_validated_at,lgu_validation_notes',
                'actualHarvestReport.lguValidator:id,name,email,municipality',
            ])
